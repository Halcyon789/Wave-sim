...
# Fixed user via query parameter
query_user = st.query_params.get("user", ["guest"])[0]
current_user = query_user if query_user else "guest"
...
# Create a mutable reference to cached list
...
st.caption("Real-time shared chat using ?user=username & synced log.")
