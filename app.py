...
st.title("ψ(t) Emotional-Cognitive Wave Visualization + Auto Resonance")

# Google Drive JSON 불러오기
...
if not junseo_df.empty and not kristina_df.empty:
    ...
    avg_resonance = np.round(1 - np.mean(delta_psi), 2)  # 1에서 Δψ 평균을 빼서 공명도 계산
    st.markdown(f"### Resonance Index (공명 지수): **{avg_resonance * 100:.1f}%**")
